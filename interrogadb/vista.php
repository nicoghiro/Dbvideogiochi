...
try {
    ...
    $sql = 'select * from giochi WHERE Genere = :type;';
    $statement = $conn->prepare($sql);
    $statement->bindParam(':type', $_GET["Genere"], PDO::PARAM_STR);
    $statement->execute();
    ...
    <tr>
      ...
      <th scope="col">Piattaforma</th>
         ...
         <tbody>';
  foreach ($data as $row) {
    echo '<tr>'.'<td>'.$row['Titolo'].'</td>'.'<td>'. $row['Genere'].'</td>'.'<td>'. $row['Piattaforma'].'</td>'.'</tr>' ;
  }
  echo  '</tbody>
        </table>';
