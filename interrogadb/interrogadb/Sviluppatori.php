...
try {
    ...
    if(isset($_GET["Casa"])){
      $sql = 'SELECT G.Titolo AS NomeGioco, S.Nome AS NomeSviluppatore, S.Sede AS SedeSviluppatore 
    ...
    $sql = 'SELECT G.Titolo AS NomeGioco, S.Nome AS NomeSviluppatore, S.Sede AS SedeSviluppatore FROM GiochiSviluppatori GS JOIN Giochi G ON GS.IdGioco = G.IdGioco JOIN Sviluppatori S ON GS.IdSviluppatore = S.IdSviluppatore;';
    $statement = $conn->prepare($sql);
    $statement->execute();
    $data = $statement->fetchAll();
    }
    ...
    <tr>
      ...
      <th scope="col">Sede Sviluppatore</th>
         ...
         <tbody>';
  foreach ($data as $row) {
    echo '<tr>'.'<td>'.$row['NomeGioco'].'</td>'.'<td>'. $row['NomeSviluppatore'].'</td>'.'<td>'. $row['SedeSviluppatore'].'</td>'.'</tr>' ;
  }
  echo  '</tbody>
        </table>';
